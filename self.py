...
def SEND_MESSAGE(op):  
        ...
        if msg.contentType == 0:
            ...
            if agtxt.lower() == 'speedtest':speedtest(acorp,'')
            ...
            if agtxt.lower().startswith('comment s'):
                ...
                wait["comment"] = text
                backupjson_1()
                line.sendMessage(acorp," 「 Autolike Comment」\nType: Comment\nStatus: Success\nFrom: "+a.title()+"\nTo: "+text.title())                                         
...
def speedtest(to,text):
    start = time.time()                              
    elapsed_time = time.time() - start
    took = time.time() - start
    line.sendMessage(to,"「 Speed 」\n - Took : %.3fms\n - Taken: %.10f" % (took,elapsed_time))
def speed(to,text):
    get_speed_time_start = time.time()    
    get_speed = time.time()      
    get_speed_time = time.time() - get_speed_time_start
    line.sendMessage(to, "Sending up to %.10f/ms" % (get_speed_time/4))    
def debug(to,text):
...
def help(to,text):
    ...
    zx = {'S':xlen, 'E':xlen2, 'M':"uc01e543dea7fe2d04b8c5141edc3a749"}
    zx2.append(zx)
    zxc += pesan2
    text = zxc+"\n║ • Selfbot Edition\n╚════════════"
